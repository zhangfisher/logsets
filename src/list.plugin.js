     ...
     return { 
        show(title,items=[],options={}){ 
            ...
            items.forEach((item,index)=>{
                ...
                if(!itemData) return
                // 显示序号                
                let order = String(index+1).padEnd(2)
                ...
                if(item.description){
                    ...
                    memo.split("\n").forEach((line,index)=>{
                        consoleOutput(opts.indent+opts.item.indent+indent+logsets.getColorizedTemplate(logsets.getColorizer(itemData.style[1])(line),...args))
                    })
                }
