...
function shwoErrorStack(error, indent = "  ") {
...
function createTaskList(context, options) {
  ...
  // 显示任务标题 ? bright
  if (opts.title) {
    const titleColorizer = logsets.getColorizer(opts.style);
...
function createTasks(logsetContext, tasks = [], options = {}) {
  ...
  if (!Array.isArray(tasks) && typeof tasks !== "function")
    throw new TypeError("tasks must be an array or a function");
  ...
  return {
    run: async (title, context) => {
      ...
      while (true) {
        const taskInfo = await getTasks(taskIndex++, setEnd);
        if (taskInfo == undefined) break;
        const taskTitle = Array.isArray(taskInfo.title)
          ? taskInfo.title
          : [taskInfo.title];
        if (taskInfo == "-") {
          // 分隔线
          ...
          try {
            ...
            let [status, tip] = result
              ? Array.isArray(result)
                ? result
                : result in taskList.options.status
                ? [result]
                : ["complete", result]
              : ["complete"];
            ...
            if (customError) {
              if (typeof taskInfo.error == "function") {
                customError = await taskInfo.error.call(ctx, {
                  error,
                  context: ctx,
                });
              }
              [errStatus, errTip] = customError
                ? Array.isArray(customError)
                  ? customError
                  : customError in taskList.options.status
                  ? [customError]
                  : ["error", customError]
                : ["error"];
            ...
            shwoErrorStack(error, `${taskList.options.indent}`);
          }
        }
